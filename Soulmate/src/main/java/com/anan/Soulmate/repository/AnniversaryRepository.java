...
public interface AnniversaryRepository extends CrudRepository<Anniversary, Long>{
	...
	@Query(value="select * from anniversary where Year(anniDate) = :anniYear and "
			+ "Month(anniDate) = :anniMonth and soulmateId = :soulmateId", nativeQuery = true)
	public List<Anniversary> findByYearAndMonth(int anniYear, int anniMonth, Long soulmateId);
}
